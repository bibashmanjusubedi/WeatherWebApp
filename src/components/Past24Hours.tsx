...
import { Cloud, CloudMoon, CloudSun, History, Sun, Wind } from "lucide-react";
...
function PastHourCard({ item }: { item: HourWeather }) {
  ...
  return (
    ...
    >
      ...
      <Icon
        className={`mt-6 size-8 drop-shadow-sm ${item.iconClassName}`}
        strokeWidth={1.8}
      />
      <p className="mt-5 text-lg font-bold tracking-[-0.04em] text-slate-900">
        {item.temp}
      </p>
